...
import { buildLoginRedirectPath, getSafeAdminReturnPath, getSafeAuthReturnPath } from "./auth-redirect";
...
assert.equal(getSafeAdminReturnPath("?next=%2Fadmin%2Fshop%2Forders"), "/admin/shop/orders");
assert.equal(getSafeAdminReturnPath("?next=%2Faccount%2Fprofile"), "/admin");
assert.equal(getSafeAdminReturnPath("?next=https%3A%2F%2Fevil.example"), "/admin");
